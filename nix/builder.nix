...
}:
{
  build-and-copy = writeScriptBin "build-and-copy" ''
    #!${fish}/bin/fish
    ${builtins.readFile ./build-and-copy.fish}
  '';
  ...
  buildDataPack =
    ...
    let
      ...
      packFormats =
        let
          groupedPackFormats = builtins.groupBy (
      ...
      packFormat =
        if packMeta.pack ? supported_formats then
          if builtins.isList packMeta.pack.supported_formats then
            {
      ...
      maxGameVersion =
        (selectFormatVersion (releases: versions: builtins.head (releases ++ versions)) packFormat.max).id;
      ...
      minGameVersion =
        (selectFormatVersion (releases: versions: lib.last (versions ++ releases)) packFormat.min).id;
      ...
      defaultNameFormat =
        ...
        let
          ...
          maxGameVersionWithZero =
            if !builtins.isNull (builtins.match "[[:digit:]]\\.[[:digit:]][[:digit:]]?" maxGameVersion) then
              "${maxGameVersion}.0"
            else
              maxGameVersion;

          gameVersionRange =
            if minGameVersion == maxGameVersion then
              "${maxGameVersionWithZero}"
            else
              "${minGameVersion}-${maxGameVersionWithZero}";
        in
        "${name}+v${version}+mc${gameVersionRange}";

      formatName = if !builtins.isNull packNameFormat then packNameFormat else defaultNameFormat;
    in
